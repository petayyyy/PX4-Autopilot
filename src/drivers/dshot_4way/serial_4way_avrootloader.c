...
// PX4 port: sample at the CENTRE of each bit (not 3/4). Our start-bit edge
// detection has more latency than Betaflight's inline IO, which pushed the
// original 3/4 sample point toward the trailing bit edge and caused misframing.
#define START_BIT_TIME      (BIT_TIME_HALVE)
//#define STOP_BIT_TIME     ((BIT_TIME * 9) + BIT_TIME_HALVE)

/* diagnostics: how often the ESC actually pulled the line low (start bit)
 * vs. we timed out waiting. Reset/read from dshot_4way_ops.c. */
volatile uint32_t g_suart_startbits = 0;
volatile uint32_t g_suart_timeouts  = 0;
volatile uint32_t g_suart_framing_err = 0;
volatile uint8_t  g_suart_rx[32];
volatile uint32_t g_suart_rx_n = 0;
...
static uint8_t suart_getc_(uint8_t *bt)
{
    ...
    // IRQs off for the whole byte: guarantees zero jitter between detecting the
    // start-bit falling edge and capturing start_time (the source of misframing).
    // hrt keeps counting with IRQs disabled; the 2ms start-bit timeout bounds it.
    ...
    g_suart_startbits++;  // line went low within the window = start bit seen
    start_time = micros();
    ...
    if ((bitmask & 1) || (!(bitmask & (1 << 9)))) {
        g_suart_framing_err++;
        if (g_suart_rx_n < 32) { g_suart_rx[g_suart_rx_n++] = (uint8_t)(bitmask >> 1); }
        return 0;
    }
    *bt = bitmask >> 1;
    if (g_suart_rx_n < 32) { g_suart_rx[g_suart_rx_n++] = *bt; }
    return 1;
}

static void suart_putc_(const uint8_t *tx_b)
{
    // whole byte (~520us) is timing-critical -> IRQs off for its duration
    irqstate_t flags = enter_critical_section();
...
/* Bring a continuously-powered BLHeli_S (SiLabs) ESC into its bootloader.
 *
 * A running BLHeli_S application only jumps to the bootloader (flash 0x1C00)
 * from its init_no_signal routine: after it loses a valid throttle signal it
 * samples the input, and if the line stays HIGH for ~15ms it does the jump;
 * if the line goes LOW in that window it restarts the app instead. So we must
 * hold the one-wire line solidly idle-HIGH long enough for the app to (a)
 * declare signal loss and (b) see a clean >15ms HIGH window, and only then
 * send the wake string (BL_ConnectEx) while the freshly-entered bootloader is
 * scanning. We retry because the app<->init_no_signal timing is not aligned
 * with us on the first try. */
uint8_t BL_ConnectBootloader(uint8_32_u *pDeviceInfo)
{
    /* Each try: hold the line cleanly idle-HIGH long enough for the app to
     * (re-)detect signal loss (~20-30ms) AND then see a >15ms HIGH window so
     * it commits the jump to the bootloader, THEN send the wake string while
     * the freshly-entered bootloader is scanning (its window is ~250ms). A
     * preceding wake-string's low pulses kick the app back to the application,
     * so the hold must be long enough to re-arm entry on every attempt. */
    for (int attempt = 0; attempt < 25; attempt++) {
        ...
        while ((uint32_t)(micros() - t0) < 80000) {   /* 80ms clean idle-high */
            ESC_SET_HI;
        }

        /* Hold IRQs off across the *entire* send+receive handshake, not just
         * per byte: an ISR firing in an inter-byte gap delays our start-bit
         * detection and corrupts that byte, and a single bad byte fails the
         * whole 9-byte connect read. The nested enter/leave in suart_*
         * keep IRQs disabled throughout. ~14ms/attempt is fine for a one-shot. */
        irqstate_t flags = enter_critical_section();
        uint8_t ok = BL_ConnectEx(pDeviceInfo);
        leave_critical_section(flags);
...
/* Diagnostic: send the BLHeli wake string, then passively sample the one-wire
 * line for listen_us microseconds (no bit decoding). Reports whether the ESC
 * pulled the line low at all, which distinguishes "ESC never entered the
 * bootloader" (lows==0) from "bootloader responds but our RX bit-timing is
 * off" (lows>0 but connect fails). Leaves the pin as input+pullup. */
uint32_t BL_DiagProbe(uint32_t listen_us, uint32_t *first_low_us, uint32_t *total_samples)
{
    /* hold idle-high so the app hands off to the bootloader first */
    ESC_OUTPUT;
    ESC_SET_HI;
    uint32_t th = micros();

    while ((uint32_t)(micros() - th) < 60000) {
        ESC_SET_HI;
    }

    uint8_t BootInit[] = {0, 0, 0, 0, 0, 0, 0, 0, 0x0D, 'B', 'L', 'H', 'e', 'l', 'i', 0xF4, 0x7D};
    BL_SendBuf(BootInit, 17);   /* BL_SendBuf leaves the pin as ESC_INPUT (pullup) */
    ...
    while ((uint32_t)(micros() - t0) < listen_us) {
        samples++;

        if (ESC_IS_LO) {
            if (lows == 0) {
                first = (uint32_t)(micros() - t0);
            }
